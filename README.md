...
内容包括:
- 程序化建模、骨骼绑定、17 个动作片段(含一段 6 秒的跳舞)。
...
| 双击狐狸 | 跳跃 |
| 工具栏「跳舞」 | 跳一段 6 秒的舞:左右律动、原地转圈、举手欢呼、鞠躬 |
...
tools/                  建模 / 绑定 / 动画 / 导出流水线(Python + Blender bpy)
  ...
  fox_build/            config(比例、颜色、骨骼、爪子坐标系)、shapes(SDF 形体)、parts(颜色与权重)、
                        ...
                        poses(FK / IK / 带碰撞的手臂求解器)、clips(17 个动作)、layers(2D 分层)、logo、export
...
- **建模方式**:用 numpy 写有符号距离场(SDF)描述各部件,平滑并集得到毛绒玩具般的圆润形体。然后经过 marching cubes、pymeshlab 各向同性重网格,再把顶点投影回 SDF 表面,并用 SDF 梯度计算法线。
  ...
  - 手臂从身体两侧、围巾下方长出,扁而逐渐加宽,末端是圆润的连指手套式爪子,爪背到爪尖有两道柔和的手指纹(和脚趾一样);脚底是干净的亮橙色(没有肉垫)。
- **颜色**:
...
- **动作(17 个)**:
  ...
  | `Pet` | 被抚摸 | — |
  | `LookBack` | 回头看尾巴 | — |
  | `Dance` | 跳舞:律动 + 交替举手、跳起转一圈、举手欢呼、鞠躬 | — |
  | `Type` | 在魔法键盘上打字(循环) | — |

- **动作的生成方式**:
  - 动作由关键姿势 + 程序化叠加生成,每帧一个线性关键帧,不会因为贝塞尔插值过冲而抖动。
  - 爪子用**带碰撞的手臂求解器**摆放:在命中目标位置、爪子朝向和掌心朝向的同时,让手臂不穿进身体和头。
  - 肩膀长在身体两侧;手伸到胸前时肩膀会向前收拢(最多 45°),合掌、比心时两只爪子能碰到一起。
  - 手腕和前臂不做扭转(线性蒙皮扭转会把前臂捏扁);需要掌心朝上/朝下时,由整条手臂在肩部转动完成。求解器同时偏向离绑定姿势最近的解,相邻关键姿势之间手臂不会突然翻转。
...
> 2D 版已停止更新:分层贴图来自较早一版的 3D 模型(当时还有手指,耳朵和手臂位置也是旧的),也没有跳舞动作。需要时可以用下面的命令从当前模型重新生成分层。
